...
- **극한 CPU 최적화**: 21.2% → 5.4% (74% 감소)
- **메모리 최적화**: 효율적인 버퍼 관리 및 memoryview 활용
- **안정적 품질**: 640x480@20fps, MJPEG 70% (CPU 최적화 버전)
...
### 블랙박스 녹화
```bash
./start_blackbox.sh dual-640    # 듀얼 카메라 녹화
```

### 웹 스트리밍 (CPU 74% 최적화)
```bash
source venv/bin/activate && python streaming_server_fixed.py
# http://라즈베리파이IP:8000/ 접속
```

### ⚠️ 현재 제한사항
- **스트리밍 중 별도 녹화 불가**: 카메라 리소스 충돌
- **다중 클라이언트**: 한 번에 1개 클라이언트만 지원
- **해결책**: 스트리밍 또는 녹화 중 하나만 선택 사용

### 🎯 최적화 성과
- **CPU 사용률**: 21.2% → **5.4%** (74% 감소)
- **화면 안정성**: 2분 후 깨짐 → **완전 안정**
- **메모리 효율**: 효율적 버퍼 관리로 성능 향상

...
def start_camera_stream(camera_id: int, resolution: str = None):
    ...
    cmd = [
        ...
        "--nopreview",
        "--codec", "mjpeg",
        "--quality", "75" if resolution == "1920x1080" else "80",
        "--flush", "1",
        "--hflip",  # 좌우 반전 (거울모드)
        "--output", "-"
    ]
...
def generate_mjpeg_stream(camera_id: int, client_ip: str = None):
    ...
    # 해상도별 최적화 파라미터 (1080p 지원 추가)
    if current_resolution == "1920x1080":
        chunk_size = 65536  # 64KB 청크 (1080p용)
        buffer_limit = 3 * 1024 * 1024  # 3MB 버퍼 (1080p용)
        buffer_keep = 1536 * 1024  # 1.5MB 유지
        frame_min_size = 10000  # 10KB
        frame_max_size = 1000000  # 1MB
        cleanup_threshold = 200000  # 200KB
        cleanup_keep = 50000  # 50KB
    elif is_720p:
...
async def root():
    ...
    <html>
    <head>
        <title>HD 듀얼 카메라 토글 (1080p 지원)</title>
        <meta charset="UTF-8">
        <style>
            body { 
            ...
            .video-container.resolution-1080 {
                max-height: 85vh;
                max-width: 100%;
                margin: 5px auto;
            }
    ...
    <body>
        <div class="container">
            <h1>HD 듀얼 카메라 토글 스트리밍 (1080p 지원)</h1>
            ...
            <div class="controls">
                ...
                <div class="control-section">
                    ...
                    <button class="resolution-btn" id="res-1080-btn" onclick="changeResolution('1920x1080')">
                        📺 1080p (1920×1080)
                    </button>
                </div>
            ...
            <!-- 온도 모니터링 영역 (1080p용) -->
            <div class="network-quality" id="thermal-monitor" style="display: none;">
                <div><strong>🌡️ CPU Temperature:</strong> <span id="cpu-temp">--°C</span></div>
                <div class="quality-bar" id="temp-warning" style="font-size: 12px; color: #dc3545;">
                    ⚠️ 1080p 스트리밍은 냉각 시스템이 필요합니다 (80°C 이상 시 성능 저하)
                </div>
            </div>
            
            <p>메모리 누수 방지, 실시간 모니터링 및 1080p HD 스트리밍 지원 버전입니다</p>
        </div>
        ...
        <script>
            ...
            function changeResolution(resolution) {
                // 1080p 선택 시 온도 모니터링 표시/숨김
                const thermalMonitor = document.getElementById('thermal-monitor');
                if (resolution === '1920x1080') {
                    thermalMonitor.style.display = 'block';
                    startThermalMonitoring();
                } else {
                    thermalMonitor.style.display = 'none';
                ...
                fetch(`/api/resolution/${resolution}`, { method: 'POST' })
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('res-1080-btn').classList.toggle('active', resolution === '1920x1080');
                            
                            // 비디오 컨테이너 클래스 업데이트
                            const videoContainer = document.getElementById('video-container');
                            let containerClass = 'video-container ';
                            if (resolution === '640x480') {
                                containerClass += 'resolution-640';
                            } else if (resolution === '1280x720') {
                                containerClass += 'resolution-720';
                            } else if (resolution === '1920x1080') {
                                containerClass += 'resolution-1080';
                            }
                            videoContainer.className = containerClass;
            ...
            function updateStats() {
                fetch('/api/stats')
                    ...
                    .then(data => {
                        ...
                        document.getElementById('res-1080-btn').classList.toggle('active', data.resolution === '1920x1080');
                        ...
                        if (videoContainer) {
                            ...
                            if (data.resolution === '640x480') {
                                containerClass += 'resolution-640';
                            } else if (data.resolution === '1280x720') {
                                containerClass += 'resolution-720';
                            } else if (data.resolution === '1920x1080') {
                                containerClass += 'resolution-1080';
                            }
                            videoContainer.className = containerClass;
                        }
            ...
            // 온도 모니터링 관련 변수
            let thermalMonitorInterval = null;
            
            function startThermalMonitoring() {
                if (thermalMonitorInterval) return; // 이미 실행 중이면 무시
                
                console.log('Starting thermal monitoring for 1080p mode...');
                thermalMonitorInterval = setInterval(updateCpuTemperature, 3000); // 3초마다 온도 체크
                updateCpuTemperature(); // 즉시 한 번 실행
            }
            
            function stopThermalMonitoring() {
                if (thermalMonitorInterval) {
                    clearInterval(thermalMonitorInterval);
                ...
                fetch('/api/cpu-temp')
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            // 온도별 경고 색상 설정
                            cpuTempElement.style.color = '#007bff';
                            tempWarning.style.color = '#6c757d';
                            
                            if (temp >= 85) {
                                cpuTempElement.style.color = '#dc3545';
                                tempWarning.style.color = '#dc3545';
                                tempWarning.textContent = '🔥 위험: CPU 온도가 너무 높습니다! 즉시 냉각 필요';
                            } else if (temp >= 80) {
                                cpuTempElement.style.color = '#fd7e14';
                                tempWarning.style.color = '#fd7e14';
                                tempWarning.textContent = '⚠️ 경고: CPU 온도 상승, 성능 저하 시작 (냉각 권장)';
                            } else if (temp >= 70) {
                                cpuTempElement.style.color = '#ffc107';
                                tempWarning.style.color = '#ffc107';
                                tempWarning.textContent = '🌡️ 주의: 온도 상승 중 (냉각 시스템 확인 권장)';
                            } else {
                                tempWarning.textContent = '✅ 정상: 1080p 스트리밍에 적합한 온도입니다';
                                tempWarning.style.color = '#28a745';
                            }
                        } else {
                            cpuTempElement.textContent = '--°C';
                            cpuTempElement.style.color = '#6c757d';
                            tempWarning.textContent = '❌ 온도 측정 실패 (vcgencmd 명령어 필요)';
                            tempWarning.style.color = '#dc3545';
                        }
                    })
                    .catch(error => {
                        ...
                        document.getElementById('temp-warning').textContent = '❌ 온도 모니터링 오류';
                    });
            }
...
@app.get("/api/cpu-temp")
async def get_cpu_temperature():
    """CPU 온도 조회 - 1080p 모드 시 thermal 모니터링용"""
    try:
        # vcgencmd를 사용해 라즈베리파이 CPU 온도 측정
        ...
        if result.returncode == 0:
            # 'temp=49.4'C' 형태에서 온도 값 추출
            temp_str = result.stdout.strip()
            if 'temp=' in temp_str:
                temp_value = float(temp_str.split('=')[1].replace("'C", ""))
                
                logger.info(f"[THERMAL] CPU 온도: {temp_value}°C")
                
                return {
                    "success": True,
                    "temperature": temp_value,
                    "unit": "celsius",
                    "warning": temp_value >= 80,
                    "critical": temp_value >= 85
                }
            else:
        ...
        logger.error("[THERMAL] vcgencmd 명령어 타임아웃")
        return {
            "success": False,
            "error": "Temperature measurement timeout",
            "message": "vcgencmd command timed out"
        }
    except subprocess.CalledProcessError as e:
        logger.error(f"[THERMAL] vcgencmd 명령어 실행 실패: {e}")
        return {
            "success": False,
            "error": "vcgencmd command failed",
            "message": "Make sure vcgencmd is available on this system"
        }
    except Exception as e:
        logger.error(f"[THERMAL] 온도 측정 오류: {e}")
        return {
            "success": False,
...
if __name__ == "__main__":
    ...
    print("[INIT] Starting HD CCTV toggle camera server on port 8002")
    print("[INIT] Access web interface at: http://<your-pi-ip>:8002")
    print("[INIT] Supports 1080p HD streaming with optimized buffers")

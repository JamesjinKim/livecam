...
bool testCameraBasic(int cameraIndex, int width, int height, bool verbose) {
    ...
    // Just test if camera can be started
    std::cout << "Testing camera start/stop..." << std::endl;
    
    // Quick test with rpicam-hello
    std::ostringstream testCmd;
    testCmd << "rpicam-hello --camera " << cameraIndex 
            << " --timeout 1000 --nopreview > /dev/null 2>&1";

...
std::string RpiCameraCapture::buildRpiCamCommand() const {
    ...
    if (config_.format == "yuv420") {
        ...
        cmd << " --codec h264";
        if (config_.verbose) {
            std::cout << "Using default H.264 encoder" << std::endl;
        }
    } else if (config_.format == "mp4") {
        // MP4 직접 저장 - 기본 설정
        cmd << " --codec libav";
        cmd << " --libav-format mp4";
...
bool RpiCameraCapture::startRpiCamProcess() {
    ...
    // 출력 설정
    if (command.find("--output") == std::string::npos) {
        command += " --output -";  // stdout으로 출력
    }
...
std::vector<int> RpiCameraCapture::listCameras() {
    ...
    // 빠른 카메라 확인: rpicam-hello --list-cameras 사용
    FILE* pipe = popen("rpicam-hello --list-cameras 2>/dev/null | grep -c '^[0-9] :'", "r");
    if (pipe) {
        char buffer[128];
        if (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
            int numCameras = std::atoi(buffer);
...
bool RpiCameraCapture::testCamera(int cameraIndex) {
    // 더 빠른 테스트: 카메라 리스트로 확인
    auto cameras = listCameras();
    return std::find(cameras.begin(), cameras.end(), cameraIndex) != cameras.end();
}

...
class CameraManager:
    ...
    # 직접 캡처 방식으로 변경 - _capture_frames 메서드 제거
    ...
    def generate_stream(self, client_ip: str):
        ...
        if not picam2:
            logger.error(f"[ERROR] 카메라 {self.current_camera} 인스턴스 없음")
            return
        ...
        try:
            while True:
                try:
                    # 카메라가 중지되었는지 확인
                    if self.current_camera not in self.camera_instances:
                        logger.info(f"[STREAM] 카메라 {self.current_camera} 중지됨, 스트림 종료")
                        break
                    ...
                    if not frame_data:
                        logger.warning(f"[WARN] 카메라 {self.current_camera}에서 데이터 없음")
                        break
